/**
 * Theme preset registry — admin/server source of truth for each theme's
 * identity (label/concept/desc) and config patch (colors, typography,
 * structure, board). The complete design-system CSS for each preset is
 * generated by `lib/theme-engine.js` (`THEME_RECIPES` + `renderThemeCss`).
 *
 * To add a theme: add a preset here AND a recipe with the same id in
 * `lib/theme-engine.js`.
 */
